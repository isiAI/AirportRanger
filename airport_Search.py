...
def plot_airports(punkte_im_umkreis):
    ...
    fig2 = px.scatter_mapbox(punkte_im_umkreis, lat=punkte_im_umkreis.geometry.y, lon=punkte_im_umkreis.geometry.x,
                             hover_name="icao", hover_data=["icao", "name"], zoom=3, height=600)
    fig2.update_layout(mapbox_style="open-street-map", margin={"r": 0, "t": 0, "l": 0, "b": 0})
    st.plotly_chart(fig2)


def main():
    st.write('### Airport Ranger')
    st.write('Welcome to the Airport Range Calculator, a handy tool for flight simulator enthusiasts to estimate '
    ...
    speed_kts = st.sidebar.number_input(max_value=1000, min_value=0, step=1, label='Speed in ktas')
    speed_mach = st.sidebar.number_input(max_value=3.0, min_value=0.0, step=0.01, label='Speed in Mach')
    ftime = st.sidebar.number_input(max_value=3680, min_value=1, step=1, label='Time to fly in minutes')
    ...
    st.sidebar.write("---")
    st.sidebar.write("Idea: __Max__ / Coding: __Gnasher__")
    st.sidebar.write('Airport Data: [Alexander Bilz](https://github.com/lxndrblz/Airports)')
    st.sidebar.write("---")
    st.sidebar.image('rangelogo.png')
